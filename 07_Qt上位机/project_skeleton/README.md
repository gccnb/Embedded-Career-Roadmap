...
这个目录给出一个“工业串口/Modbus 调试上位机”的推荐项目骨架。它用于指导后续实现：界面怎么分区、模块怎么拆、日志怎么保存、面试时怎么讲。

建议把它和 `07_Qt上位机/09_Modbus调试上位机项目.md`、`07_Qt上位机/qt_modbus_debugger_demo/README.md`、`08_项目实战/02_Modbus_RTU设备调试项目.md` 一起阅读。`project_skeleton` 讲结构，`qt_modbus_debugger_demo` 给出最小源码，项目实战文档负责把下位机和上位机联调串起来。
