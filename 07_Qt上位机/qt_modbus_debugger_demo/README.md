...
7. 没有真实下位机时，可以在模拟 RX 输入框中填入响应帧，例如 `01 03 04 00 19 00 64 2A 1F`。
8. 点击“模拟 RX”，观察解析表格和日志区。
9. 点击“导出日志”，把当前日志保存为 CSV 文件。
...
| --- | --- |
| 时间 | 解析事件发生时间 |
| 类型 | `03`、`06`、`ERROR`、`EXCEPTION` 等类型 |
| 摘要 | 面向人阅读的简短结论 |
| 详情 | 原始帧、字节数、地址和值等细节 |

## 面试表达

> 基于 Qt 编写 Modbus RTU 串口调试上位机最小 Demo，拆分串口收发、Modbus 03/06 组帧、CRC16 校验、模拟 RX 输入、解析结果表格、TX/RX 日志显示和 CSV 日志导出模块，可配合下位机模拟寄存器完成通信链路验证和问题复盘。
